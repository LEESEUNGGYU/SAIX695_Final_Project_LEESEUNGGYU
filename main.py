...
def Test_phase(model, args, k):
    ...
    for i in range(TEST_SIZE):
        for episode in test_loader:
            ...
            """ TEST Method """
            # shot embedding
            shot_emb = model(data_shot)

            #print(shot_emb.shape)

            # average each shot to make prototype
            shot1_emb = torch.mean(shot_emb[0:5,:],0) 
            shot2_emb = torch.mean(shot_emb[5:10,:],0) 
            shot3_emb = torch.mean(shot_emb[10:15,:],0) 
            shot4_emb = torch.mean(shot_emb[15:20,:],0) 
            shot5_emb = torch.mean(shot_emb[20:25,:],0) 
            #print(shot1_emb.shape)
           
            # concatenate & reshape
            proto_emb = torch.cat([shot1_emb, shot2_emb, shot3_emb, shot4_emb, shot5_emb])
            proto_emb = proto_emb.reshape((5,shot1_emb.size()[0]))

            query_emb = model(data_query)
            #print(proto_emb.shape, query_emb.shape)
            # caculate distances using Square Euclidean Metric
            logits = square_euclidean_metric(query_emb, proto_emb)
            #loss_indi = torch.nn.CrossEntropyLoss()
            #loss = loss_indi(-logits, labels)
...
def train(args):
    ...
    for i in range(TOTAL):
        ...
        if (i+1) % SAVE_FREQ == 0:
            PATH = 'checkpoints_Adamax/%d_%s.pth' % (i + 1, args.name)
...
if __name__ == '__main__':
    ...
    if not os.path.isdir('checkpoints_Adamax'):
        os.mkdir('checkpoints_Adamax')
